...
interface PostBodyProps {
  ...
  postId: string;
  onClose: () => void;
  onShowEngagements: (type: 'likes' | 'reposts') => void;
}

export default function PostBody({ post, postId, onClose, onShowEngagements }: PostBodyProps) {
  ...
  return (
    <div className="px-4 pt-4">
              ...
              <span className="text-[--text-muted] text-sm">
                ...
                {post.metadata?.confidence !== undefined && (
                  <span
                    ...
                    aria-label={`${Math.round(post.metadata.confidence * 100)}% confidence`}
                  >
      ...
      {post.media_urls && post.media_urls.length > 0 && (
        <div
          className={`grid ${post.media_urls.length === 1 ? 'grid-cols-1' : 'grid-cols-2'} gap-0.5 mt-4 rounded-2xl overflow-hidden border border-white/10`}
          role="group"
      ...
      <div className="flex items-center gap-6 py-4 mt-2 border-t border-b border-white/10">
        ...
        {post.repost_count > 0 ? (
          <button
            onClick={() => onShowEngagements('reposts')}
        ...
        {post.like_count > 0 ? (
          <button
            onClick={() => onShowEngagements('likes')}
            className="flex items-center gap-1 hover:underline"
          >
      ...
      {/* Action buttons */}
      <div
        className="flex items-center justify-around py-2 border-b border-white/10"
        role="group"
      ...
      >
        ...
        >
          <svg
            className="w-[22px] h-[22px] text-[--text-muted] group-hover:text-[--success]"
            viewBox="0 0 24 24"
            fill="currentColor"
            aria-hidden="true"
          >
        ...
        >
          <svg
            className="w-[22px] h-[22px] text-[--text-muted] group-hover:text-[--like]"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="1.5"
        ...
        >
          <svg
            className={`w-[22px] h-[22px] ${bookmarked ? 'text-[--info]' : 'text-[--text-muted] group-hover:text-[--info]'}`}
            viewBox="0 0 24 24"
        ...
        >
          <svg
            className="w-[22px] h-[22px] text-[--text-muted] group-hover:text-[--info]"
            viewBox="0 0 24 24"
            fill="currentColor"
            aria-hidden="true"
          >
