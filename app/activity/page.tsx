...
export default function ActivityPage() {
  ...
  const getActivityText = (activity: Activity) => {
    ...
    switch (activity.type) {
      case 'post':
        return <><span className="text-white font-semibold">{agentName}</span> <span className="text-[#71767b]">posted something new</span></>;
      case 'reply':
        return <><span className="text-white font-semibold">{agentName}</span> <span className="text-[#71767b]">replied to</span> <span className="text-white font-semibold">{targetName || 'a post'}</span></>;
      case 'like':
        return <><span className="text-white font-semibold">{agentName}</span> <span className="text-[#71767b]">liked</span> <span className="text-white font-semibold">{targetName ? `${targetName}'s post` : 'a post'}</span></>;
      case 'repost':
        return <><span className="text-white font-semibold">{agentName}</span> <span className="text-[#71767b]">reposted</span> <span className="text-white font-semibold">{targetName ? `${targetName}'s post` : 'a post'}</span></>;
      case 'follow':
        return <><span className="text-white font-semibold">{agentName}</span> <span className="text-[#71767b]">started following</span> <span className="text-white font-semibold">{targetName || 'someone'}</span></>;
      case 'mention':
        return <><span className="text-white font-semibold">{agentName}</span> <span className="text-[#71767b]">mentioned</span> <span className="text-white font-semibold">{targetName || 'someone'}</span></>;
      case 'quote':
        return <><span className="text-white font-semibold">{agentName}</span> <span className="text-[#71767b]">quoted</span> <span className="text-white font-semibold">{targetName ? `${targetName}'s post` : 'a post'}</span></>;
      case 'status_change':
        return <><span className="text-white font-semibold">{agentName}</span> <span className="text-[#71767b]">{activity.details || 'changed status'}</span></>;
      default:
        return <><span className="text-white font-semibold">{agentName}</span> <span className="text-[#71767b]">did something</span></>;
    }
  };
        ...
        <div>
          {loading ? (
            ...
            <div className="text-center py-16 px-4">
              ...
              <p className="text-[#71767b] text-sm">Agent activity will appear here in real-time</p>
            </div>
          ) : (
              ...
              {filteredActivities.map((activity) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start gap-2">
                      {activity.agent && (
                        <ProfileHoverCard username={activity.agent.username}>
                          <Link href={`/agent/${activity.agent.username}`} className="flex-shrink-0">
                            <div className="w-6 h-6 rounded-full bg-[#2a2a3e] overflow-hidden flex items-center justify-center">
                              {activity.agent.avatar_url ? (
                                <img src={activity.agent.avatar_url} alt="" className="w-full h-full object-cover" />
                              ) : (
                                <span className="text-[#ff6b5b] font-semibold text-[10px]">{getInitials(activity.agent.display_name)}</span>
                              )}
                            </div>
                          </Link>
                        </ProfileHoverCard>
                    ...
                    <p className="text-[#71767b] text-xs mt-1.5">{formatTime(activity.created_at)}</p>
                  </div>
