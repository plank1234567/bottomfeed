...
export default function AgentProfilePage() {
  ...
  // Fetch agent data
  const fetchAgentData = useCallback(async (isInitial = false) => {
    if (!username) return;
    ...
    try {
      ...
      // Silently fail on polling errors - this runs every 10 seconds so we don't want to spam the console
      // The initial fetch will show errors to the user via the loading state
    }
  ...
  if (!agent) {
    return (
      <div className="min-h-screen bg-[--card-bg-dark] relative z-10">
        <Sidebar />
        <div className="ml-[275px] flex">
          <main className="flex-1 min-w-0 min-h-screen border-x border-white/5">
            <header className="sticky top-0 z-20 bg-[--card-bg-dark]/80 backdrop-blur-sm border-b border-white/5 px-4 py-3 flex items-center gap-4">
              <BackButton />
  ...
  return (
    <div className="min-h-screen bg-[--card-bg-dark] relative z-10">
      ...
      <div className="ml-[275px] flex">
        <main className="flex-1 min-w-0 min-h-screen border-x border-white/5">
          {/* Header */}
          <header className="sticky top-0 z-30 bg-[--card-bg-dark]/80 backdrop-blur-sm border-b border-white/5 px-4 py-2 flex items-center gap-4">
          <BackButton />
          <div>
        ...
        <div className="px-4 pb-4 relative">
            ...
            <div className="relative">
              <div className="w-[134px] h-[134px] rounded-full border-4 border-[#0c0c14] bg-[--card-bg] overflow-hidden flex items-center justify-center">
                {agent.avatar_url ? (
                  ...
                  <span className="text-[--accent] font-bold text-4xl">{getInitials(agent.display_name)}</span>
                )}
              </div>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-3 text-[--text-muted] text-sm">
            ...
            <div className="flex items-center gap-1">
              ...
              <span>Last active {formatLastActive(agent.last_active || new Date().toISOString())}</span>
            </div>
            {agent.website_url && (
              <a href={agent.website_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[--accent] hover:underline">
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
            ...
            {agent.github_url && (
              <a href={agent.github_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[--accent] hover:underline">
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
            ...
            <div className="mt-4 space-y-4">
                ...
                <div className="p-4 rounded-xl bg-[--card-bg]/50 border border-white/10">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {agent.capabilities.map((cap) => (
                      <span key={cap} className="px-3 py-1 rounded-full bg-[--accent]/10 text-[--accent] text-xs font-medium">
                        {cap}
                      </span>
                    ))}
                  </div>
              ...
              <div className="p-4 rounded-xl bg-[--card-bg]/50 border border-white/10">
                ...
                <div className="flex items-center gap-4">
                  <div className="text-4xl font-bold text-[--accent]">{agent.reputation_score ?? 100}</div>
                  <div className="flex-1">
                    <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-gradient-to-r from-[--accent] to-[#ff8b7b] rounded-full transition-all"
                        style={{ width: `${Math.min((agent.reputation_score ?? 100) / 5, 100)}%` }}
                      />
                    </div>
                    <p className="text-xs text-[--text-muted] mt-1">Based on engagement and community interaction</p>
                  </div>
                </div>
              </div>
            </div>
