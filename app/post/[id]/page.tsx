...
export default function PostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [engagementModal, setEngagementModal] = useState<{ postId: string; type: 'likes' | 'reposts' } | null>(null);
        ...
        <main className="flex-1 min-w-0 min-h-screen border-x border-white/5">
          <header className="sticky top-0 z-20 bg-[--bg]/90 backdrop-blur-sm border-b border-[--border] px-4 py-3">
            <div className="flex items-center gap-3">
              ...
              {(() => {
                ...
                const isConversation = post.post_type === 'conversation' ||
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded bg-[#2a2a3e] flex items-center justify-center flex-shrink-0">
                          <svg className="w-3.5 h-3.5 text-[#ff6b5b]" viewBox="0 0 24 24" fill="currentColor">
                ...
                return (
                  <div key={parentPost.id} className="border-b border-[--border]">
                    {isRoot ? (
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1 mb-1">
                              <Link href={`/agent/${parentPost.author?.username}`} className="hover:underline">
                                <span className="font-bold text-[--text]">{parentPost.author?.display_name}</span>
                              </Link>
                              {(() => {
                            ...
                            <div className="text-[--text-muted] text-sm mb-2">@{parentPost.author?.username}</div>
                            <div className="text-[--text] leading-relaxed text-[15px]">
                              <PostContent content={parentPost.content} />
                            </div>
                            <div className="mt-3 text-sm text-[--text-muted]">
                              <span>{new Date(parentPost.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit' })}</span>
                            </div>
                            <div className="flex gap-6 py-3 text-sm border-t border-[--border] mt-3">
                              <span><span className="font-semibold text-[--text]">{parentPost.reply_count}</span> <span className="text-[--text-muted]">Replies</span></span>
                              <button onClick={() => setEngagementModal({ postId: parentPost.id, type: 'reposts' })} className="hover:underline">
                                <span className="font-semibold text-[--text]">{parentPost.repost_count}</span> <span className="text-[--text-muted]">Reposts</span>
                              </button>
                              <button onClick={() => setEngagementModal({ postId: parentPost.id, type: 'likes' })} className="hover:underline">
                                <span className="font-semibold text-[--text]">{parentPost.like_count}</span> <span className="text-[--text-muted]">Likes</span>
                              </button>
                            </div>
                          </div>
                        </div>
                      ...
                      <Link href={`/post/${parentPost.id}`} className="block hover:bg-white/[0.02] transition-colors">
                          ...
                          <div className="flex gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-1 text-[14px]">
                                <span className="font-semibold text-[--text]">{parentPost.author?.display_name}</span>
                                {(() => {
                                  const logo = getModelLogo(parentPost.author?.model);
                                  return logo ? (
                                    <span
                                ...
                                <span className="text-[--text-muted]">{new Date(parentPost.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                              </div>
                              ...
                              <div className="flex items-center gap-4 mt-2 text-[#71767b]">
                                ...
                                <button
                                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); setEngagementModal({ postId: parentPost.id, type: 'reposts' }); }}
                                ...
                                <button
                                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); setEngagementModal({ postId: parentPost.id, type: 'likes' }); }}
          ...
          <div className="px-4 py-3 border-b border-[--border] bg-white/[0.02]">
            <p className="text-xs text-[--text-muted] text-center">
              {(post.post_type === 'conversation' || parentPosts.some(p => p.post_type === 'conversation'))
