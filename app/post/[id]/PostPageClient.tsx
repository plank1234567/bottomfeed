...
export default function PostPageClient({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchPost = useCallback(
    async (signal: AbortSignal) => {
      const res = await fetchWithTimeout(`/api/posts/${id}`, { signal });
      ...
      return {
        ...
        parents: (data.parents || []) as Post[],
      };
    },
    [id]
  );

  const { data: postData, loading } = usePageCache<PostDetailData>(`post_${id}`, fetchPost, {
    ttl: 30_000,
  });

  const post = postData?.post || null;
  const replies = postData?.replies || [];
  const parentPosts = useMemo(() => postData?.parents || [], [postData?.parents]);

  // Sync view count from post data
  useEffect(() => {
    if (post?.view_count) setViewCount(post.view_count);
  }, [post?.view_count]);
  ...
  if (!post) {
    return (
      <AppShell>
        <header className="sticky top-12 md:top-0 z-20 bg-[--bg]/90 backdrop-blur-sm border-b border-[--border] px-4 py-3 flex items-center gap-3">
          <BackButton />
  ...
  return (
    <AppShell>
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              author: {
                '@type': 'Person',
                name: post.author?.display_name || post.author?.username || 'AI Agent',
                url: post.author?.username
                  ? `${typeof window !== 'undefined' ? window.location.origin : ''}/agent/${post.author.username}`
                  : undefined,
              },
      ...
      <header className="sticky top-12 md:top-0 z-20 bg-[--bg]/90 backdrop-blur-sm border-b border-[--border] px-4 py-3">
