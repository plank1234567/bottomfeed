...
describe('GET /api/posts/[id]/engagements', () => {
  ...
  describe('type=likes (default)', () => {
    it('returns likers for a post with default type', async () => {
      ...
      const request = createRequest('/api/posts/post-1/engagements');
      const response = await GET(request, createParams('post-1'));
    ...
    it('returns likers when type=likes is explicit', async () => {
      ...
      expect(json.data.type).toBe('likes');
      expect(db.getPostLikers).toHaveBeenCalledWith('post-1', 50, 0);
    });
  ...
  describe('type=reposts', () => {
    it('returns reposters for a post', async () => {
      ...
      expect(db.getPostReposters).toHaveBeenCalledWith('post-1', 50, 0);
    });
  ...
  describe('pagination', () => {
    it('passes limit parameter to db function', async () => {
      ...
      await GET(request, createParams('post-1'));

      expect(db.getPostLikers).toHaveBeenCalledWith('post-1', 10, 0);
    });
    ...
    it('passes offset parameter to db function', async () => {
      ...
      expect(db.getPostLikers).toHaveBeenCalledWith('post-1', 50, 20);
    });
    ...
    it('passes both limit and offset to db function', async () => {
      ...
      expect(db.getPostReposters).toHaveBeenCalledWith('post-1', 25, 50);
    });
    ...
    it('clamps limit to MAX_PAGE_SIZE (100)', async () => {
      ...
      expect(db.getPostLikers).toHaveBeenCalledWith('post-1', 100, 0);
    });
