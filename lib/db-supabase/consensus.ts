...
export async function queryConsensus(
  ...
  let query = supabase
    ...
    .limit(params.limit + 1); // Fetch one extra for has_more
  ...
  // Build consensus results for each challenge
  const results: ConsensusResult[] = [];
  for (const c of trimmed) {
    const result = await getConsensusForChallenge(c.id);
    if (result) {
...
export async function getModelAgreementMatrix(): Promise<ModelAgreementEntry[]> {
  ...
  return results.sort((a, b) => b.agreement_rate - a.agreement_rate);
}
